...
const { FridgeIngredients } = require("../../db/models");
...
const validateFridgeIng = [
  check("quantity")
    .isInt({ min: 1, max: 999 })
    .withMessage(
      "You can have between 1 and 999 of an ingredient, please enter a valid number.",
    ),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!checker.length) {
      //create it
      const fridgeIng = await FridgeIngredients.create({
        ingredientId,
        fridgeId,
        quantity,
      });
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const fridgeIngredient =
      await FridgeIngredients.findByPk(fridgeIngredientId);
