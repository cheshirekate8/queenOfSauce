...
function EditQuantityForm({ setShowModal, currIngredient }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setErrors([]);
        dispatch(kitchenActions.editQuantity(currIngredient.id, quantity, sessionUser.id))
            .then(() => {
                setShowModal(false)
            })
            .catch(async (res) => {
    ...
    return (
        <>
            <h1 className='modalTitle'>Edit quantity of {currIngredient.Ingredient.name} in your Fridge.</h1>
            <form onSubmit={handleSubmit} className='modalForm'>
                ...
                <button type="submit" className='modalBtns'>Edit Quantity</button>
            </form>
        </>
    );
}
